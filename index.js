...
// Tugas No 1
// GET REGENCIES BY PROVINCE NAME  , INPUT NAMA PROVINCE, OUTPUT LIST NAMA KOTA
...
function GetRegenciesByProvince(province) {
    ...
    for (let i=0; i<regencies.length; i++) {
        if (regencies[i].province_id == provinceId) {
            newRegencies.push(regencies[i].name)
        }
    }
    return newRegencies
}
...
// console.log(GetRegenciesByProvince("ACEH"));


// Tugas No 2 
// GET REGENCIES BY CONTAIN NAME, INPUT NAMA PROVINCE Not complete, contoh  ("DKI") , ("JAKARTA"), "Sumatera" , OUTPOUT LIST NAMA Kota

function GetRegenciesByContainName(province) {
    let provinceArr = []
    for (let i=0; i<provinces.length; i++){
        if (((provinces[i].name).toLowerCase()).includes(province.toLowerCase())) {
            provinceArr.push(provinces[i].id)
        }
    }
    ...
    for (let j=0; j<provinceArr.length; j++) {
        for (let i=0; i<regencies.length;i++){
            if(regencies[i].province_id == provinceArr[j]) {
                regenciesArr.push(regencies[i].name)
            }
        }
    }
...
// console.log(GetRegenciesByContainName("sumatera"))

/*
/ GET PROVINCE NAME BY CITY NAME, CITY NAME can be Uppercase or Lowercase , output Province Name

jakarta Timur => DKI JAKARTA
bOGoR => JAWA BARAT
*/
function GetProvinceByCityName(city) {
    let provinceID = 0
    for (let i=0; i<regencies.length; i++){
        if ((regencies[i].name).toLowerCase() == (city.toLowerCase())) {
            provinceID = regencies[i].province_id
        }
    }
...
// console.log(GetProvinceByCityName("KABUPATEN nias"));



/*
// GET REGENCIES BY WORDCOUNT, List Nama Kota yg Total kata nya sesuai input, 2 => JAKARTA BARAT,

contoh Input : 2 => output : DKI JAKARTA, KOTA BOGOR dll
contoh input 3 => Kabupaten Aceh Tenggara
*/

function GetRegenciesByWordCount(count){
    let city_province = []
    for (let i=0; i<provinces.length; i++){
        if ((provinces[i].name).split(" ").length == count) {
            city_province.push(provinces[i].name)
        }
    }

    for (let i=0; i<regencies.length; i++){
        if ((regencies[i].name).split(" ").length == count) {
            city_province.push(regencies[i].name)
        }
    }
...
// console.log(GetRegenciesByWordCount(3))